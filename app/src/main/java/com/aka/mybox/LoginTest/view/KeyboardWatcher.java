...
public class KeyboardWatcher implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private final List<SoftKeyboardStateListener> listeners = new
            LinkedList<SoftKeyboardStateListener>();
    ...
    public KeyboardWatcher(View activityRootView, boolean isSoftKeyboardOpened) {
        ...
        int resourceId = activityRootView.getContext()
                .getResources()
                .getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            //根据资源ID获取响应的尺寸值
            statusBarHeight = activityRootView.getContext()
                    .getResources()
    ...
    public void onGlobalLayout() {
        ...
        if (!isSoftKeyboardOpened && heightDiff > activityRootView.getRootView().getHeight() / 4) {
            ...
            if ((activityRootView.getContext() instanceof Activity) && !isFullScreen((Activity)
                    activityRootView
                    .getContext())) {
        ...
        } else if (isSoftKeyboardOpened && heightDiff < activityRootView.getRootView()
                .getHeight() / 4) {
